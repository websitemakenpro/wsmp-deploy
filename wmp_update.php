<?php
/**
 * WMP Updater — eenmalig uitvoeren, daarna verwijdert zichzelf
 * Toegang via: https://websitemakenpro.nl/wmp_update.php
 */
...
define('SMTP_USER', 'ece21cd984f2eb8a423519a29ea3f943');
define('SMTP_PASS', '0bdf0c14004a2d6156fcca6768f4ecfd');
...
function smtpSend($to, $subject, $htmlBody, $bcc = '') {
    $smtp = @stream_socket_client('tcp://' . SMTP_HOST . ':' . SMTP_PORT, $errno, $errstr, 15);
    if (!$smtp) { error_log("SMTP connect failed: $errstr ($errno)"); return false; }
    $read = function() use ($smtp) { return fgets($smtp, 512); };
    $send = function($cmd) use ($smtp) { fwrite($smtp, $cmd . "\r\n"); };
    $read();
    $send('EHLO websitemakenpro.nl');
    while (($line = $read()) !== false) { if (substr($line, 3, 1) === ' ') break; }
    $send('STARTTLS');
    $tlsResp = $read();
    if (strpos($tlsResp, '220') === false) { fclose($smtp); return false; }
    ...
    foreach ($recipients as $r) { $send("RCPT TO:<$r>"); $read(); }
    if ($bcc) { $send("RCPT TO:<$bcc>"); $read(); }
    $send('DATA');
    $read();
    $date = date('r');
...
function sendEmail($to, $subject, $htmlBody, $bcc = '') {
    ...
    if (!$ok) $ok = fallbackMail($to, $subject, $htmlBody, $bcc);
    return $ok;
}
...
.top{background:#0051FF;padding:24px 32px;color:#fff}.top h1{font-size:20px;margin:0 0 4px;font-weight:600}
.ref{font-family:monospace;background:rgba(255,255,255,.2);padding:3px 10px;border-radius:6px;font-size:13px}
.body{padding:28px 32px}.label{margin-bottom:12px;font-size:11px;font-weight:600;text-transform:uppercase;letter-spacing:.1em;color:#8E8E98;padding-bottom:8px;border-bottom:2px solid #f0f0eb}
table{width:100%;border-collapse:collapse}td{padding:9px 0;border-bottom:1px solid #f5f5f0;font-size:14px;color:#0F0F14}
.lbl{color:#54545F;width:42%}.total td{font-weight:700;font-size:17px;border-top:2px solid #e0e0d8;border-bottom:none;padding-top:14px;color:#0051FF}
...
<div class='body'><div class='label'>Besteldetails</div><table>
...
<tr class='total'><td>Totaal eenmalig</td><td>$totalHTML</td></tr></table>
<div style='margin-top:20px' class='label'>Klantgegevens</div><table>
...
</table><div class='action'><strong>Actie:</strong> Neem contact op + stuur factuur. Na betaling: " . ($domainIsNew ? "domein registreren via mijn.host (<strong>$domain</strong>)." : "bestaand domein koppelen.") . "</div>
</div><div class='footer'>WebsiteMakenPro · info@websitemakenpro.nl · +31 6 24741597</div></div></body></html>";
...
.logo{font-size:22px;font-weight:700;letter-spacing:-.02em;margin-bottom:16px}.logo span{font-style:italic;opacity:.85}
...
.body{padding:28px 32px}.intro{font-size:16px;color:#3A3A44;line-height:1.6;margin-bottom:24px;padding-bottom:20px;border-bottom:1px solid #f0f0eb}
...
table{width:100%;border-collapse:collapse}td{padding:10px 0;border-bottom:1px solid #f5f5f0;font-size:14px;color:#0F0F14;vertical-align:top}
.lbl{color:#54545F;width:45%;font-size:13px}.total-row td{font-weight:700;font-size:18px;border-top:2px solid #0051FF;border-bottom:none;padding-top:14px;color:#0051FF}
...
.stxt{font-size:14px;color:#3A3A44;line-height:1.5}.stxt strong{color:#0F0F14;display:block;margin-bottom:2px}
...
.cta p{color:#fff;font-size:14px;margin:0}.cta a{display:inline-block;background:#25D366;color:#fff;padding:9px 18px;border-radius:100px;font-size:13px;font-weight:600;text-decoration:none}
...
.footer a{color:#0051FF;text-decoration:none}.green{color:#12885D;font-weight:600}
</style></head><body><div class='card'>
<div class='hero'><div class='logo'>WebsiteMaken<span>pro</span></div><h1>Bedankt voor je bestelling,<br>$fname!</h1><div class='ref-badge'>$orderRef</div></div>
<div class='body'>
...
<div class='steps'><h3>Wat gebeurt er nu?</h3>
...
<div class='cta'><p>Neem gerust contact op als je vragen hebt.</p><a href='https://wa.me/31624741597?text=Hoi,%20mijn%20bestelnummer%20is%20$orderRef'>WhatsApp ons</a></div>
</div>
<div class='footer'>WebsiteMakenPro &middot; info@websitemakenpro.nl &middot; +31 6 24741597<br>
...
if (!$adminOk && !$customerOk) error_log("ORDER $orderRef: all mail delivery failed");

$logDir = __DIR__ . '/orders';
if (!is_dir($logDir)) @mkdir($logDir, 0755, true);
@file_put_contents($logDir . '/orders.log', json_encode([
    'ref' => $orderRef, 'date' => $orderDate, 'pkg' => $pkgLabel,
    'domain' => $domain, 'name' => $fullName, 'email' => $email,
    'phone' => $phone, 'total' => $totalStr,
    'adminSmtp' => $adminOk, 'customerSmtp' => $customerOk,
], JSON_UNESCAPED_UNICODE) . "\n", FILE_APPEND | LOCK_EX);

echo json_encode(['success' => true, 'orderRef' => $orderRef, 'message' => "Bestelling $orderRef ontvangen!"]);
PHPEOF;

$dir1 = __DIR__ . '/api-order';
if (!is_dir($dir1)) mkdir($dir1, 0755, true);
$ok1 = file_put_contents($dir1 . '/index.php', $apiOrder);
$results[] = ['file' => 'api-order/index.php', 'bytes' => $ok1];

// =====================================
// 2) domain-check-api/index.php  
// =====================================
$domainCheck = <<<'PHPEOF'
<?php
/**
...
if (!$name || !preg_match('/^[a-z0-9]([a-z0-9\-]{0,61}[a-z0-9])?$/', $name)) {
    http_response_code(400); echo json_encode(['error' => 'Ongeldige domeinnaam.']); exit;
}

$tlds = array_slice(array_filter(array_map('trim', explode(',', $tldsParam))), 0, 6);
if (empty($tlds)) { http_response_code(400); echo json_encode(['error' => 'Geen TLDs opgegeven.']); exit; }
...
function checkDomain($name, $tld) {
    ...
    if (function_exists('curl_init')) {
        ...
        curl_setopt_array($ch, [
            ...
            CURLOPT_HTTPHEADER => ['API-Key: ' . MIJN_HOST_API_KEY, 'Accept: application/json', 'User-Agent: WebsiteMakenPro-DomainChecker/1.0'],
        ...
        curl_close($ch);
        if ($curlErr) return ['tld'=>$tld,'domain'=>$domain,'status'=>'error','message'=>$curlErr];
    } else {
        $ctx = stream_context_create(['http'=>['method'=>'GET','timeout'=>8,'header'=>"API-Key: ".MIJN_HOST_API_KEY."\r\nAccept: application/json\r\n"]]);
        $response = @file_get_contents($url, false, $ctx);
        $httpCode = 0;
        if (isset($http_response_header[0]) && preg_match('/HTTP\/\S+\s+(\d+)/', $http_response_header[0], $m)) $httpCode = (int)$m[1];
        if ($response === false) return ['tld'=>$tld,'domain'=>$domain,'status'=>'error','message'=>'API onbereikbaar'];
    }
    if ($httpCode === 200) {
        $data = @json_decode($response, true);
        $status = $data['data']['status'] ?? 'unknown';
        if ($status === 'available') return ['tld'=>$tld,'domain'=>$domain,'status'=>'available'];
        if ($status === 'taken') return ['tld'=>$tld,'domain'=>$domain,'status'=>'registered'];
    }
    return checkDomainRdap($name, $tld, $domain);
}

function checkDomainRdap($name, $tld, $domain) {
    $endpoints = ['nl'=>'https://rdap.sidn.nl/domain/','com'=>'https://rdap.verisign.com/com/v1/domain/','net'=>'https://rdap.verisign.com/net/v1/domain/','org'=>'https://rdap.publicinterestregistry.org/rdap/domain/','eu'=>'https://rdap.eurid.eu/rdap/domain/','io'=>'https://rdap.identitydigital.services/rdap/domain/'];
    $url = ($endpoints[$tld] ?? 'https://rdap.org/domain/') . rawurlencode($domain);
    if (!function_exists('curl_init')) return ['tld'=>$tld,'domain'=>$domain,'status'=>'unknown'];
    $ch = curl_init();
    curl_setopt_array($ch, [CURLOPT_URL=>$url,CURLOPT_RETURNTRANSFER=>true,CURLOPT_TIMEOUT=>6,CURLOPT_CONNECTTIMEOUT=>4,CURLOPT_FOLLOWLOCATION=>true,CURLOPT_HTTPHEADER=>['Accept: application/rdap+json, application/json','User-Agent: WebsiteMakenPro-DomainChecker/1.0'],CURLOPT_SSL_VERIFYPEER=>true,CURLOPT_SSL_VERIFYHOST=>2]);
    $response = curl_exec($ch);
    $httpCode = (int) curl_getinfo($ch, CURLINFO_HTTP_CODE);
    curl_close($ch);
    if ($httpCode === 404) return ['tld'=>$tld,'domain'=>$domain,'status'=>'available'];
    if ($httpCode === 200) {
        $data = @json_decode($response, true);
        $result = ['tld'=>$tld,'domain'=>$domain,'status'=>'registered'];
        if ($data && isset($data['events'])) foreach ($data['events'] as $ev) if (($ev['eventAction']??'') === 'expiration') $result['expires'] = $ev['eventDate']??null;
        return $result;
    }
...
$results2 = [];
foreach ($tlds as $tld) $results2[] = checkDomain($name, $tld);
echo json_encode(['name'=>$name,'results'=>$results2,'source'=>'mijn.host API','timestamp'=>gmdate('c')], JSON_UNESCAPED_UNICODE|JSON_UNESCAPED_SLASHES);
PHPEOF;

$dir2 = __DIR__ . '/domain-check-api';
if (!is_dir($dir2)) mkdir($dir2, 0755, true);
$ok2 = file_put_contents($dir2 . '/index.php', $domainCheck);
$results[] = ['file' => 'domain-check-api/index.php', 'bytes' => $ok2];

// =====================================
// 3) .htaccess voor /api/domain-check routing
// =====================================
$htaccess = "Options -Indexes\n\nRewriteEngine On\nRewriteRule ^api/domain-check$ /domain-check-api/index.php [L,QSA]\nRewriteRule ^api/order/?$ /api-order/index.php [L,QSA]\n";
$ok3 = file_put_contents(__DIR__ . '/.htaccess', $htaccess);
$results[] = ['file' => '.htaccess', 'bytes' => $ok3];

// Self-destruct
@unlink(__FILE__);

header('Content-Type: application/json');
echo json_encode(['status' => 'OK', 'updated' => $results, 'selfDeleted' => true]);
